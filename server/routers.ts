...
export const appRouter = router({
  ...
  experiments: router({
    create: protectedProcedure.input(z.object({
      materialId: z.number(), experimentName: z.string().min(1).max(255), inputParameters: z.record(z.string(), z.any()),
    ...
    updateStatus: protectedProcedure.input(z.object({ experimentId: z.string(), status: z.enum(["draft", "running", "paused", "completed", "failed"]) })).mutation(async ({ ctx, input }) => {
      if (!ctx.user) throw new TRPCError({ code: "UNAUTHORIZED" });
  ...
  simulation: router({
    run: protectedProcedure.input(z.object({
      experimentId: z.string(), materialId: z.number(), materialWeight: z.number().min(0.1).max(1000), waterContent: z.number().min(0).max(100), oilContent: z.number().min(0).max(100), targetPressure: z.number().min(1).max(1000), targetTemperature: z.number().min(20).max(150), ultrasonicFrequency: z.number().min(20).max(100), duration: z.number().min(0.5).max(24), materialWaterRatio: z.string(), processModel: z.enum(["vacuum", "distillation", "ultrasonic", "hybrid"]),
    })).mutation(async ({ ctx, input }) => {
      if (!ctx.user) throw new TRPCError({ code: "UNAUTHORIZED" });
      try {
        ...
        // Convert every physics frame into a machine-state frame. The UI can now replay
        // the same causal controller decisions instead of deriving machine state from % progress.
        const controller = new ProcessStateEngine({ targetPressureMbar: input.targetPressure, targetTemperatureC: input.targetTemperature }, {
          pressureMbar: 1013.25, temperatureC: 25, yieldPercent: 0, waterRemovedKg: 0, oilRecoveredKg: 0, energyKwh: 0,
        });
        const processTimeline: ProcessState[] = results.realTimeData.map((frame) => controller.tick({
          pressureMbar: frame.pressure,
          temperatureC: frame.temperature,
          yieldPercent: frame.yieldPercentage,
          waterRemovedKg: frame.waterRemoved,
          oilRecoveredKg: frame.oilRecovered,
          energyKwh: frame.energyConsumed,
        }, frame.timestamp));
