...
export const appRouter = router({
  ...
  simulation: router({
    run: protectedProcedure.input(z.object({
      ...
      ultrasonicFrequency: z.number().min(20).max(100),
      duration: z.number().min(0.5).max(24),
      materialWaterRatio: z.string(),
      ...
      try {
        ...
        const resultId = await db.createSimulationResult({
          ...
          oilComposition: results.oilComposition,
          energyConsumed: results.energyConsumed,
          efficiency: results.efficiency,
          wasteComposition: results.wasteComposition,
  ...
  ai: router({
    analyze: protectedProcedure.input(z.object({
      experimentId: z.string(),
      dataUpToTime: z.number().optional(),
      params: z.object({
        targetPressure: z.number(),
        targetTemperature: z.number(),
        ultrasonicFrequency: z.number(),
        duration: z.number(),
        materialWeight: z.number(),
        oilContent: z.number(),
        processModel: z.string(),
      }),
      ...
      if (result && result.realTimeData) {
        ...
        if (input.dataUpToTime !== undefined) {
