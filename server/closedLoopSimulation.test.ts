...
const config = { targetPressureMbar: 100, targetTemperatureC: 70, materialWeightKg: 10, waterContentPercent: 20, oilContentPercent: 5, dtSeconds: 1, maxSteps: 1000 };
...
describe('ClosedLoopSimulationEngine', () => {
  ...
  it('matches the closed-form vacuum baseline at frame 138', () => {
    ...
    expect(frame.sensorAfter.energyKwh).toBeCloseTo(0.204, 12);
  });
  ...
  it('reset returns the machine to deterministic initial conditions', () => {
    ...
    expect(engine.getSensors()).toEqual({ chamberSealed: true, pressureMbar: 1013.25, temperatureC: 25, yieldPercent: 0, waterRemovedKg: 0, oilRecoveredKg: 0, energyKwh: 0 });
  });
});
