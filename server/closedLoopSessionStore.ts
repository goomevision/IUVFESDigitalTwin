...
export async function saveClosedLoopSession(experimentId: string, status: ClosedLoopSessionStatus, snapshot: ClosedLoopSnapshot): Promise<ClosedLoopSessionRecord> {
  ...
  await db.execute(sql`
    ...
    ON DUPLICATE KEY UPDATE status = VALUES(status), snapshot = VALUES(snapshot), frameCount = VALUES(frameCount), lastStep = VALUES(lastStep)
  `);
...
export async function getClosedLoopSession(experimentId: string): Promise<ClosedLoopSessionRecord | null> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const result = await db.execute(sql`SELECT id, experimentId, status, snapshot, frameCount, lastStep FROM closedLoopSessions WHERE experimentId = ${experimentId} LIMIT 1`);
  ...
  return { id: String(row.id), experimentId: String(row.experimentId), status: row.status as ClosedLoopSessionStatus, snapshot: snapshot as ClosedLoopSnapshot, frameCount: Number(row.frameCount), lastStep: Number(row.lastStep) };
}
