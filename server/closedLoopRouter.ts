...
const hardwareSchema = z.object({
  ...
  ultrasonicOperatingFrequencyKHz: z.number().positive().max(1000).optional(),
  ultrasonicRequestedPowerKW: z.number().nonnegative().max(100000).optional(),
  coldTrapTemperaturesC: z.tuple([z.number(), z.number(), z.number(), z.number()]),
  coldTrapHeatTransferCoefficientWPerM2K: z.number().nonnegative().max(100000),
  ...
  vacuumPipeEffectiveLengthFactor: z.number().min(1).max(100),
  vacuumPumpOutletPressureMbar: z.number().nonnegative().max(1000),
