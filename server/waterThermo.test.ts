...
describe('water thermodynamics kernel', () => {
  ...
  it('predicts a strong vapor drive under deep vacuum at moderate temperature', () => {
    ...
    expect(state.saturationPressureMbar).toBeGreaterThan(100);
    expect(state.boilingLikely).toBe(true);
    expect(state.vaporDrive).toBeGreaterThan(0);
    expect(state.waterMassVaporizedKg).toBeGreaterThan(0);
  });

  it('does not claim boiling when pressure exceeds saturation pressure', () => {
    const state = engine.evaluate(50, 1000, 1, 1);
    expect(state.boilingLikely).toBe(false);
    expect(state.vaporDrive).toBe(0);
    expect(state.waterMassVaporizedKg).toBe(0);
  });

  it('keeps latent heat positive in the normal process range', () => {
    const low = engine.evaluate(30, 100, 0, 1);
    const high = engine.evaluate(80, 100, 0, 1);
    expect(low.latentHeatKjPerKg).toBeGreaterThan(high.latentHeatKjPerKg);
    expect(high.latentHeatKjPerKg).toBeGreaterThan(0);
  });
});
