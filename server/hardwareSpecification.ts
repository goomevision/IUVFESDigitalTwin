...
export const IUVFES_VMMES_BASELINE: IuvfesHardwareSpecification = {
  ...
  reactor: {
    ...
    workingVolume: baseline(250, 'L', 'Concept range 200–250 L; nominal baseline 250 L.'),
    operatingPressure: baseline({ min: 10, max: 200 }, 'mbar_abs'),
    operatingTemperature: baseline({ min: 30, max: 60 }, '°C'),
    internalDiameter: required(0, 'mm', 'Engineering drawing required.'),
    shellLength: required(0, 'mm', 'Engineering drawing required.'),
    wallThickness: required(0, 'mm', 'External-pressure/vacuum engineering check required.'),
    designExternalPressure: required(0, 'bar', 'Pressure-vessel engineering input required.'),
    designTemperature: required(0, '°C', 'Design-condition input required.'),
    note: 'Vacuum shell stability must not be reduced to a simple internal-pressure thickness formula.',
  },
  heating: {
    id: 'HJ-001',
    type: 'HEATING_JACKET',
    ...
    medium: required('', undefined, 'Heating medium and thermal circuit required.'),
    note: 'Use energy balance, heat-transfer coefficient, thermal inertia and heat loss.',
  },
  ultrasonic: {
    id: 'US-001',
    type: 'ULTRASONIC_TRANSDUCER',
    ...
    note: 'Transducer count, amplitude, mounting and coupling require equipment data.',
  },
  vacuumPump: {
    id: 'VP-001',
    type: 'VACUUM_PUMP',
    ...
    pumpCurve: required('', undefined, 'Vendor pump curve required.'),
