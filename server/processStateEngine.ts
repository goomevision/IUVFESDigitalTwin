...
export class ProcessStateEngine {
  ...
  public tick(sensors: MachineSensors, elapsedSeconds: number): ProcessState {
    ...
    if (interlocks.pressureTransient || interlocks.temperatureTransient) {
      this.state.alarm = interlocks.pressureTransient
        ? 'PRESSURE_TRANSIENT: process rate exceeded configured simulation envelope.'
        : 'TEMPERATURE_TRANSIENT: process rate exceeded configured simulation envelope.';
    }
  ...
  private initialState(sensors: MachineSensors, reason: string): ProcessState {
    const interlocks = this.evaluateInterlocks(sensors, undefined, 1);
    return { stage: 'PRE_FLIGHT', progress: 0, elapsedSeconds: 0, sensors: { ...sensors }, commands: this.off(), interlocks, alarm: null, transitionReason: reason };
  }

  private evaluateInterlocks(sensors: MachineSensors, previous: MachineSensors | undefined, dtSeconds: number): InterlockState {
    const safety = evaluateSafety(previous, sensors, dtSeconds, {
      minPressureMbar: this.config.minPressureMbar,
      maxPressureMbar: this.config.maxPressureMbar,
      maxTemperatureC: this.config.maxTemperatureC,
      maxPressureRateMbarPerSecond: this.config.maxPressureRateMbarPerSecond,
      maxTemperatureRateCPerSecond: this.config.maxTemperatureRateCPerSecond,
    });
    ...
    return {
      chamberSealed: sensors.chamberSealed,
      pressureSafeForHeating,
      temperatureSafeForCooling,
      overTemperature: safety.overTemperature,
      vacuumAchieved,
