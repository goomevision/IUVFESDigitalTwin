...
export function runGoldenPatchouliExperiment(overrides: Partial<ClosedLoopSimulationConfig> = {}): GoldenPatchouliDataset {
  const config: ClosedLoopSimulationConfig = {
    ...
    dtSeconds: 1, maxSteps: 600, realTime: false,
    hardware: { chamberVolumeL: 250, pumpCapacityM3h: 200, thermalMassKJPerC: 250, heatingPowerKW: 9, coolingPowerKW: 3, vacuumLineConductanceFactor: 1, vacuumLineDiameterM: 0.02, leakRateMbarPerSecond: 0 },
    ...overrides,
  };
...
function toGoldenRow(frame: CausalFrame): GoldenPatchouliRow {
  ...
  return {
    ...
    massKg: Number(m.totalTrackedMassKg ?? 0), waterKg: Number(m.moistureKg ?? 0), vaporKg: Number(m.vaporKg ?? 0), condensateWaterKg: Number(m.condensateWaterKg ?? 0),
    oilInMatrixKg: Number(m.oilInMatrixKg ?? 0), oilVaporKg: Number(m.oilVaporKg ?? 0), recoveredOilKg: Number(m.recoveredOilKg ?? 0), volatileLossKg: Number(m.volatileLossKg ?? 0),
    ...
    safetyStatus: frame.safety.severity,
  };
}
