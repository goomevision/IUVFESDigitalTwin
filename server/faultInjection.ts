...
export interface FaultInjectionMetrics {
  ...
  finalPressureMbar: number;
  finalTemperatureC: number;
  finalYieldPercent: number;
  finalEnergyKwh: number;
  minimumPressureMbar: number;
  maximumPressureMbar: number;
  maximumTemperatureC: number;
  safetyEventCount: number;
  criticalSafetyEventCount: number;
}

export interface FaultInjectionRun {
...
export interface FaultInjectionCampaign {
  baseline: FaultInjectionRun;
  scenarios: FaultInjectionRun[];
}
...
export function buildFaultHardwareProfile(
  base: VirtualHardwareDynamicsConfig,
  faults: readonly FaultInjection[],
): VirtualHardwareDynamicsConfig {
...
function summarize(result: ClosedLoopResult): FaultInjectionMetrics {
  ...
  const pressures = frames.map((frame) => frame.sensorAfter.pressureMbar);
  const temperatures = frames.map((frame) => frame.sensorAfter.temperatureC);
  const last = result.finalSensors;
  const critical = new Set(['CRITICAL', 'EMERGENCY']);

  return {
    status: result.status,
    steps: frames.length,
    simulatedSeconds: frames.at(-1)?.timestampSeconds ?? 0,
    finalPressureMbar: last.pressureMbar,
    finalTemperatureC: last.temperatureC,
    finalYieldPercent: last.yieldPercent,
    finalEnergyKwh: last.energyKwh,
    minimumPressureMbar: pressures.length ? Math.min(...pressures) : last.pressureMbar,
    maximumPressureMbar: pressures.length ? Math.max(...pressures) : last.pressureMbar,
    maximumTemperatureC: temperatures.length ? Math.max(...temperatures) : last.temperatureC,
    safetyEventCount: result.safetyEvents.length,
    criticalSafetyEventCount: result.safetyEvents.filter((event) => critical.has(event.severity)).length,
  };
}

export function runFaultInjectionScenario(
...
export function runFaultInjectionCampaign(
  baseConfig: ClosedLoopSimulationConfig,
  scenarios: readonly FaultInjectionScenario[],
): FaultInjectionCampaign {
  const baseline = runFaultInjectionScenario(baseConfig, {
    id: 'baseline',
