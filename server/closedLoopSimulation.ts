...
export interface ClosedLoopResult {
  ...
  finalMaterialInventory: MaterialInventory;
  pausedSteps: number[];
...
export class ClosedLoopSimulationEngine {
  ...
  private readonly material: MaterialProcessEngine;
  ...
  constructor(private readonly config: ClosedLoopSimulationConfig) {
    ...
    this.material = new MaterialProcessEngine({
      materialMassKg: config.materialWeightKg,
      initialWaterFraction: Math.max(0, config.waterContentPercent / 100),
      initialOilFraction: Math.max(0, config.oilContentPercent / 100),
    });
    this.control = new ProcessControlLoop();
  }

  private initialSensors(): MachineSensors {
    return { chamberSealed: true, pressureMbar: 1013.25, temperatureC: 25, yieldPercent: 0, waterRemovedKg: 0, oilRecoveredKg: 0, energyKwh: 0 };
  }
  ...
  public reset(): void {
    ...
    this.material.restore(new MaterialProcessEngine({
      materialMassKg: this.config.materialWeightKg,
      initialWaterFraction: Math.max(0, this.config.waterContentPercent / 100),
      initialOilFraction: Math.max(0, this.config.oilContentPercent / 100),
    }).snapshot());
  }
  ...
  private advanceStep(now: number): CausalFrame {
    ...
    const materialInventory = this.material.step({
      materialMassKg: this.config.materialWeightKg,
      initialWaterFraction: this.config.waterContentPercent / 100,
      initialOilFraction: this.config.oilContentPercent / 100,
      chamberPressureMbar: physicalSensorAfter.pressureMbar,
      materialTemperatureC: physicalSensorAfter.temperatureC,
      heaterPowerFraction: intendedCommands.heater ? 1 : 0,
      vacuumPowerFraction: effectiveCommands.vacuumPump ? 1 : 0,
      extractorPowerFraction: effectiveCommands.extractor ? 1 : 0,
      condenserPowerFraction: effectiveCommands.condenser ? 1 : 0,
      coolingPowerFraction: effectiveCommands.cooling ? 1 : 0,
      dtSeconds: this.dtSeconds,
    });
    const sensorWithMaterial = {
      ...physicalSensorAfter,
      waterRemovedKg: Math.min(this.target.waterRemovedKg, materialInventory.condensateWaterKg),
      oilRecoveredKg: Math.min(this.target.oilRecoveredKg, materialInventory.recoveredOilKg),
      yieldPercent: Math.min(100, materialInventory.oilRecoveryFraction * 100),
    };
    ...
    const frame: CausalFrame = {
      ...
      intendedCommands,
      effectiveCommands,
      physicalSensorAfter: sensorWithMaterial,
  ...
  public runToCompletion(): ClosedLoopResult {
    ...
    return {
      ...
      finalMaterialInventory: this.material.snapshot(),
  ...
  public getMaterialInventory(): MaterialInventory { return this.material.snapshot(); }
  ...
  public restore(snapshot: ClosedLoopSnapshot): void {
    ...
    this.state.restore(snapshot.state); this.dynamics.restore(snapshot.dynamics); this.material.restore(snapshot.materialInventory); this.control.restore(snapshot.control);
  }
}
