...
export class ClosedLoopSimulationEngine {
  ...
  private coolingTemperatureC: number; private sensors: MachineSensors; private elapsedSeconds = 0; private stepNumber = 0; private paused = false; private readonly frames: CausalFrame[] = []; private readonly pausedSteps: number[] = [];
  ...
  public step(): CausalFrame | null {
    ...
    const physicalSensorAfter = this.dynamics.step(this.target, effectiveCommands, this.dtSeconds); this.elapsedSeconds += this.dtSeconds; this.stepNumber += 1; this.sensors = physicalSensorAfter;
    ...
    const frame: CausalFrame = { step: this.stepNumber, timestampSeconds: this.elapsedSeconds, sensorBefore, controller: controllerAfterActuation, controlOutput, intendedCommands, effectiveCommands, physicalSensorAfter: { ...physicalSensorAfter }, sensorAfter: { ...physicalSensorAfter }, materialInventory, safety, paused: false }; this.frames.push(frame); return frame;
  }
