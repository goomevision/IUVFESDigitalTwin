...
export class ClosedLoopSimulationEngine {
  ...
  private advanceStep(now: number): CausalFrame {
    ...
    const physicalSensorAfter = this.dynamics.step(this.target, effectiveCommands, this.dtSeconds, latentHeatLoadKW, vaporGenerationKgPerS);
    const materialInventory = this.material.step({ materialMassKg: this.config.materialWeightKg, initialWaterFraction: this.config.waterContentPercent / 100, initialOilFraction: this.config.oilContentPercent / 100,
      ...
      vacuumPowerFraction: effectiveCommands.vacuumPump ? 1 : 0, extractorPowerFraction: intendedCommands.extractor ? 1 : 0, condenserPowerFraction: effectiveCommands.condenser ? 1 : 0,
