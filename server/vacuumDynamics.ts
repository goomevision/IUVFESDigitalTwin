export type VacuumDynamicsInput = { timeStepS: number; absolutePressureKPa: number; vesselVolumeM3: number; gasTemperatureK: number; pumpSpeedM3PerS: number; valveOpening: number; vaporGenerationKgPerS: number; gasMolarMassKgPerMol: number; };
export type VacuumDynamicsResult = { absolutePressureKPa: number; pressureRateKPaPerS: number; gasMoles: number; pumpThroughputM3PerS: number; pressureShockIndex: number; warnings: string[] };
...
export function stepVacuumDynamics(input: VacuumDynamicsInput): VacuumDynamicsResult {
  if (input.timeStepS <= 0 || input.vesselVolumeM3 <= 0 || input.gasTemperatureK <= 0 || input.gasMolarMassKgPerMol <= 0) throw new Error("Invalid vacuum dynamics state: dt, volume, temperature and molar mass must be positive.");
  ...
  const nextN = Math.max(0, n + (generatedMolesPerS - pumpedMolesPerS) * input.timeStepS);
  ...
  return { absolutePressureKPa: nextPressurePa / 1000, pressureRateKPaPerS, gasMoles: nextN, pumpThroughputM3PerS, pressureShockIndex: shockIndex, warnings };
}
