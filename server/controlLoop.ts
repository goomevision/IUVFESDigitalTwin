...
export class PIDController {
  ...
  public update(setpoint: number, measurement: number, dtSeconds: number): number {
    ...
    // Basic anti-windup: only retain integral growth when it is not driving farther into saturation.
  ...
  public update(input: { targetTemperatureC: number; targetPressureMbar: number; temperatureC: number; pressureMbar: number; stage: string; dtSeconds: number }): ControlOutput {
    ...
    return {
      ...
      valve: {
        ...
        vaporToCondenser: ['EXTRACTION', 'CONDENSATION'].includes(input.stage) ? 1 : 0,
        coolingWater: ['CONDENSATION', 'COOL_DOWN'].includes(input.stage) ? 1 : 0,
      },
    };
  }
