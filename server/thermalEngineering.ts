...
/** Compatibility name used by hardware-coupling adapters. */
export type ThermalModelConfig = Pick<ThermalEngineeringInput, 'thermalMassKJPerC' | 'effectiveHeatLossKWPerC'>;
...
export function estimateHeatingTime(input: ThermalEngineeringInput): ThermalTimeEstimate {
  const errors = validate(input); if (errors.length > 0) return { reachable: false, estimatedSeconds: null, netPowerKW: 0, limitingFactor: errors.join(' ') };
  ...
  const equilibriumDelta = availableAtTarget / k; const startDelta = input.initialTemperatureC - input.ambientTemperatureC; const targetDelta = input.targetTemperatureC - input.ambientTemperatureC;
...
export function stepThermalModel(input: ThermalEngineeringInput, dtSeconds: number): ThermalStepResult {
  ...
  if (errors.length > 0 || !finite(dtSeconds) || dtSeconds < 0) return { temperatureC: input.initialTemperatureC, netHeatKW: 0, energyAddedKWh: 0, heatLossKWh: 0, coolingRemovedKWh: 0, latentHeatRemovedKWh: 0, reachedTarget: false, warning: errors.concat('dtSeconds must be non-negative and finite.').join(' ') };
  const heaterEfficiency = Math.min(1, Math.max(0, input.heaterEfficiency ?? 1)); const coolingEfficiency = Math.min(1, Math.max(0, input.coolingEfficiency ?? 1)); const latentHeatLoadKW = Math.max(0, input.latentHeatLoadKW ?? 0);
  const energyAddedKWh = input.heaterPowerKW * heaterEfficiency * dtSeconds / 3600; const coolingRemovedKWh = input.coolingPowerKW * coolingEfficiency * dtSeconds / 3600; const latentHeatRemovedKWh = latentHeatLoadKW * dtSeconds / 3600;
  const deltaAmbient = Math.max(0, input.initialTemperatureC - input.ambientTemperatureC); const heatLossKWh = input.effectiveHeatLossKWPerC * deltaAmbient * dtSeconds / 3600;
  ...
  const deltaT = (netHeatKW * dtSeconds) / input.thermalMassKJPerC; const temperatureC = input.initialTemperatureC + deltaT;
