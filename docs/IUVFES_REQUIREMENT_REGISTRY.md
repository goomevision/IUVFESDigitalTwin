...
| RQ-009 | A browser refresh can recover an experiment’s persisted runtime without re-creating the session. | `closedLoop.getForExperiment` and `ProcessSimulator` now discover and adopt an authorized persisted session. | Runtime-store recovery test passes; authenticated browser refresh proof is still unavailable. | **PENDING_RUNTIME** |
| RQ-010 | Replay uses structurally valid causal frames and refuses to fabricate missing telemetry. | Replay reads only `closedLoop.replayForExperiment`, then rejects structurally incomplete frames. | TypeScript and full regression suite pass; authenticated UI replay remains pending. | **PENDING_RUNTIME** |
| RQ-011 | The same causal-frame range produces stable replay evidence integrity. | Canonical replay evidence excludes volatile `exportedAt` metadata. | `ExperimentReplay.test.ts` verifies deterministic canonical-body serialization. | **VERIFIED (unit)** |
...
| RQ-017 | Scientific records preserve the source IUVFES experiment and do not fabricate a default sample identity. | `resolveScientificRecordIdentity()` and `ScientificRunRecorder` pass the original experiment id and use `UNKNOWN` when no sample id is supplied. | Identity unit tests and TypeScript validation pass. | **VERIFIED (unit)** |
