...
|---|---|---|---|---|
| G-001 | Git source of truth | **BLOCKED** | The advertised remote branch ref did not match the local tracking ref during forensic inspection; local `todo.md` is also modified. | Preserve the worktree and reconcile the authoritative remote ref before a push, merge, or rebase. No destructive Git operation is permitted. |
...
| G-003 | Session recovery from UI refresh | **OPEN** | `ensureRuntimeSession()` supports a cache-miss lookup by session id and an experiment-id fallback, but `ProcessSimulator` keeps `sessionId` only in local component state and always invokes `closedLoop.create` from `START`. | Add a typed, authorized discovery/recovery path from experiment to persisted session, then prove refresh/resume behaviour. |
| G-004 | Replay evidence determinism | **OPEN** | `ExperimentReplay.exportEvidence()` includes `generatedAt` inside the hashed canonical JSON payload. The same selected frame range therefore produces a different checksum at a different export time. | Define a stable canonical evidence body that excludes volatile export metadata, then add a deterministic checksum regression test. |
