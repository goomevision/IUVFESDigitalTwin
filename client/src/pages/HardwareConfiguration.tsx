...
export default function HardwareConfiguration() {
  ...
  const setTrapArray = (key: "coldTrapHeatTransferAreasM2" | "coldTrapVolumesL" | "coldTrapCondensateCapacityKg", index: number, value: string) => {
    const next = [...config[key]] as [number, number, number, number];
    next[index] = Number(value);
    setConfig((current) => ({ ...current, [key]: next }));
    setSaved(false);
  };
  const geometricVolumeL = Math.PI * (config.reactorInternalDiameterMm / 2000) ** 2 * (config.reactorShellLengthMm / 1000) * 1000;
  const checks = useMemo(() => [
    ["Geometry → derived internal volume", config.reactorInternalDiameterMm > 0 && config.reactorShellLengthMm > 0],
    ["Working volume → chamber dynamics", config.chamberVolumeL > 0],
    ["Pipe diameter + length → conductance", config.vacuumPipeDiameterMm > 0 && config.vacuumPipeLengthM > 0],
    ["Pump capacity → effective pumping speed", config.pumpCapacityM3h >= 0],
    ...
    ["Cold-trap area/temperature → condensation load", config.coldTrapHeatTransferCoefficientWPerM2K >= 0 && config.coldTrapHeatTransferAreasM2.every((v) => v >= 0)],
    ["Ultrasonic installed max → acoustic limit", config.ultrasonicFrequencyKHz > 0 && config.ultrasonicMaxPowerKW > 0],
  ] as const, [config]);
  ...
  return <div className="min-h-screen bg-[radial-gradient(circle_at_top,#10263a_0%,#050912_48%,#02040a_100%)] p-4 text-slate-100 md:p-6"><div className="mx-auto max-w-[1500px] space-y-4">
    ...
    <div className="grid gap-4 xl:grid-cols-[1.25fr_1fr_0.8fr]">
      <section className="rounded-2xl border border-cyan-500/20 bg-slate-950/70 p-5"><h2 className="mb-4 text-lg font-semibold text-cyan-300">1. VACUUM REACTOR — VR-001</h2><div className="grid grid-cols-2 gap-3">
        ...
        <label className="text-xs text-slate-400">Working Volume (L)<input className={fieldClass} type="number" value={config.chamberVolumeL} onChange={(e) => setNumber("chamberVolumeL", e.target.value)} /><span className="mt-1 block text-[10px] text-cyan-500/70">Geometric cylinder estimate: {geometricVolumeL.toFixed(1)} L</span></label>
      ...
      </div><div className="mt-4 grid grid-cols-2 gap-3 rounded-xl border border-slate-800 bg-slate-900/50 p-3 text-xs"><div><span className="text-slate-500">Geometry</span><div className="mt-1 text-emerald-300">DERIVED + SESSION CONTRACT</div></div><div><span className="text-slate-500">Structural safety</span><div className="mt-1 text-yellow-300">ENGINEERING REVIEW REQUIRED</div></div></div></section>

      <section className="rounded-2xl border border-cyan-500/20 bg-slate-950/70 p-5"><h2 className="mb-4 text-lg font-semibold text-cyan-300">2. VACUUM LINE + INSTALLED CAPACITY</h2><div className="space-y-3">
        <label className="block text-xs text-slate-400">Main Vacuum Pipe ID (mm)<input className={fieldClass} type="number" min="1" value={config.vacuumPipeDiameterMm} onChange={(e) => setNumber("vacuumPipeDiameterMm", e.target.value)} /></label>
        <label className="block text-xs text-slate-400">Main Vacuum Pipe Length (m)<input className={fieldClass} type="number" min="0.1" value={config.vacuumPipeLengthM} onChange={(e) => setNumber("vacuumPipeLengthM", e.target.value)} /></label>
        <label className="block text-xs text-slate-400">Effective Length Factor<input className={fieldClass} type="number" min="1" step="0.1" value={config.vacuumPipeEffectiveLengthFactor} onChange={(e) => setNumber("vacuumPipeEffectiveLengthFactor", e.target.value)} /></label>
        <label className="block text-xs text-slate-400">Pump Outlet Pressure (mbar)<input className={fieldClass} type="number" min="0" value={config.vacuumPumpOutletPressureMbar} onChange={(e) => setNumber("vacuumPumpOutletPressureMbar", e.target.value)} /></label>
      ...
      <section className="rounded-2xl border border-cyan-500/20 bg-slate-950/70 p-5"><h2 className="mb-4 text-lg font-semibold text-cyan-300">3. ULTRASONIC LIMIT</h2><div className="space-y-3"><label className="block text-xs text-slate-400">Installed Frequency (kHz)<input className={fieldClass} type="number" value={config.ultrasonicFrequencyKHz} onChange={(e) => setNumber("ultrasonicFrequencyKHz", e.target.value)} /></label><label className="block text-xs text-slate-400">Installed Max Power (kW)<input className={fieldClass} type="number" value={config.ultrasonicMaxPowerKW} onChange={(e) => setNumber("ultrasonicMaxPowerKW", e.target.value)} /></label><div className="rounded-xl border border-purple-500/20 bg-purple-500/5 p-3 text-xs text-purple-200">Operating frequency/power remain dynamic controls. This screen only defines the installed ceiling.</div></div></section>
    </div>

    <section className="rounded-2xl border border-cyan-500/20 bg-slate-950/70 p-5"><h2 className="mb-4 text-lg font-semibold text-cyan-300">4. COLD TRAP SYSTEM — CT-001 … CT-004</h2><div className="mb-4 rounded-xl border border-slate-800 bg-slate-900/40 p-3 text-xs text-slate-400">Set the physical trap temperature, heat-transfer area, vessel volume and condensate inventory. Overall heat-transfer coefficient is required for the thermal-load model; zero means <span className="text-yellow-300">DATA GAP</span>, not zero physical performance.</div><div className="mb-4 max-w-sm"><label className="block text-xs text-slate-400">Overall Heat-Transfer Coefficient U (W/m²/K)<input className={fieldClass} type="number" min="0" value={config.coldTrapHeatTransferCoefficientWPerM2K} onChange={(e) => setNumber("coldTrapHeatTransferCoefficientWPerM2K", e.target.value)} /></label></div><div className="grid gap-3 md:grid-cols-4">{config.coldTrapTemperaturesC.map((value, i) => <div key={i} className="rounded-xl border border-cyan-500/10 bg-slate-900/50 p-3"><div className="mb-3 font-mono text-xs text-cyan-300">CT-00{i + 1}</div><label className="block text-xs text-slate-400">Trap Temp (°C)<input className={fieldClass} type="number" value={value} onChange={(e) => setTrap(i, e.target.value)} /></label><label className="mt-2 block text-xs text-slate-400">Area (m²)<input className={fieldClass} type="number" min="0" value={config.coldTrapHeatTransferAreasM2[i]} onChange={(e) => setTrapArray("coldTrapHeatTransferAreasM2", i, e.target.value)} /></label><label className="mt-2 block text-xs text-slate-400">Volume (L)<input className={fieldClass} type="number" min="0" value={config.coldTrapVolumesL[i]} onChange={(e) => setTrapArray("coldTrapVolumesL", i, e.target.value)} /></label><label className="mt-2 block text-xs text-slate-400">Condensate Capacity (kg)<input className={fieldClass} type="number" min="0" value={config.coldTrapCondensateCapacityKg[i]} onChange={(e) => setTrapArray("coldTrapCondensateCapacityKg", i, e.target.value)} /></label></div>)}</div></section>

    <section className="rounded-2xl border border-cyan-500/20 bg-slate-950/70 p-5"><div className="mb-4 flex items-center justify-between"><div><h2 className="text-lg font-semibold text-cyan-300">5. WIRING / PROVENANCE STATUS</h2><p className="text-xs text-slate-500">Green means the value is present in the hardware session contract; it does not mean structural certification.</p></div>{saved && <span className="font-mono text-xs text-emerald-300">SAVED • READY FOR NEW SESSION</span>}</div><div className="grid gap-2 md:grid-cols-2 xl:grid-cols-5">{checks.map(([label, ok]) => <div key={label} className="flex items-center gap-2 rounded-lg border border-slate-800 bg-slate-900/50 p-3 text-xs"><CheckCircle2 className={`h-4 w-4 ${ok ? "text-emerald-400" : "text-red-400"}`} /><span>{label}</span></div>)}</div></section>

    <section className="rounded-2xl border border-cyan-500/20 bg-slate-950/70 p-5"><div className="mb-4"><h2 className="text-lg font-semibold text-cyan-300">6. SYSTEM HARDWARE MAP</h2><p className="text-xs text-slate-500">Static specification → session contract → derived geometry/limits → physics inputs where the current engine supports the parameter.</p></div><div className="grid items-center gap-2 md:grid-cols-7">{["VR-001 REACTOR", "VACUUM LINE", "HJ-001 HEATER", "US-001 ULTRASONIC", "VP-001 PUMP", "CT-001..004 TRAPS", "SAFETY"].map((item, i) => <div key={item} className="flex items-center gap-2"><div className="min-h-20 flex-1 rounded-xl border border-cyan-500/20 bg-slate-900/60 p-3 text-center"><div className="text-xs font-semibold text-cyan-200">{item}</div><div className="mt-2 text-[10px] text-emerald-300">SESSION CONTRACT</div></div>{i < 6 && <ArrowRight className="hidden h-4 w-4 shrink-0 text-cyan-400 md:block" />}</div>)}</div></section>
