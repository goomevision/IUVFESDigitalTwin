...
export function ProcessMachine3D({ machine }: Props) {
  const mountRef = useRef<HTMLDivElement | null>(null); const stateRef = useRef(machine); stateRef.current = machine;
  useEffect(() => {
    ...
    const vaporCurve = new THREE.CatmullRomCurve3([new THREE.Vector3(-2.7, 2.5, 0.0), new THREE.Vector3(-0.7, 2.5, 0.0), new THREE.Vector3(0.4, 3.0, -0.9), new THREE.Vector3(5.2, 3.0, -1.45)]);
    ...
    const animate = () => {
      ...
      trapGroups.forEach((group, i) => { const data = state?.coldTraps?.[i]; const active = condensing || cooling; const temp = data?.temperatureC; const coldFactor = typeof temp === "number" ? Math.max(0, Math.min(1, (25 - temp) / 105)) : 0.35; trapBodies[i].material.emissive.setHex(active ? (coldFactor > 0.65 ? 0x082f49 : 0x10243a) : 0x061522); trapCoils[i].material.color.setHex(active ? (coldFactor > 0.65 ? 0x60a5fa : 0x38bdf8) : 0x2563eb); group.position.y = 3.25 + Math.sin(time * 0.5 + i) * 0.015; });
      chamberParticles.children.forEach((p, i) => { const offset = p.userData.offset as number; const travel = (time * (vacuum || extracting ? 0.09 : 0.015) + offset) % 1; p.position.set(-1.0 + travel * 1.9, -1.35 + ((i * 0.47) % 2.7), 0.4 + Math.sin(i * 1.7 + time) * 0.5); p.visible = vacuum || extracting || hot; p.scale.setScalar(0.45 + vacuumLevel * 1.2 + thermal * 0.7); });
  ...
  return <div className="relative h-[590px] overflow-hidden rounded-2xl border border-cyan-500/20 bg-slate-950/90 shadow-2xl shadow-cyan-950/20"><div ref={mountRef} className="absolute inset-0" />
    <div className="pointer-events-none absolute inset-x-0 top-0 flex items-start justify-between bg-gradient-to-b from-slate-950/95 via-slate-950/65 to-transparent p-4"><div><div className="flex items-center gap-2 font-mono text-[10px] font-semibold tracking-[0.25em] text-cyan-300"><span className={`h-2 w-2 rounded-full ${fault ? "bg-red-400" : hasEngineFrame ? "bg-emerald-400" : "bg-slate-500"}`} />3D PROCESS MACHINE</div><div className="mt-1 text-[10px] text-slate-500">LIVE ENGINE STATE → VISUAL PROCESS MODEL</div></div><div className={`rounded-lg border px-3 py-2 font-mono text-[9px] ${fault ? "border-red-500/30 bg-red-500/10 text-red-300" : hasEngineFrame ? "border-emerald-500/20 bg-emerald-500/5 text-emerald-300" : "border-slate-700 bg-slate-950/80 text-slate-500"}`}>{fault ? "SAFETY TRIP" : hasEngineFrame ? stage : "WAITING FOR ENGINE FRAME"}</div></div>
    <div className="pointer-events-none absolute left-4 top-20 space-y-1.5 font-mono text-[9px]"><div className="rounded bg-slate-950/70 px-2 py-1 text-slate-400">REACTOR</div><div className="rounded bg-slate-950/70 px-2 py-1 text-slate-400">VACUUM PUMP</div><div className="rounded bg-slate-950/70 px-2 py-1 text-slate-400">VAPOR / CONDENSATION</div></div>
    <div className="pointer-events-none absolute right-4 top-20 rounded-xl border border-white/10 bg-slate-950/80 p-3 font-mono text-[9px] shadow-xl backdrop-blur"><div className="mb-2 text-[8px] tracking-[0.2em] text-slate-500">LIVE CONNECTIONS</div><div className="space-y-1.5 text-slate-400"><div><span className={commands?.vacuumPump ? "text-cyan-300" : "text-slate-600"}>●</span> VACUUM LINE</div><div><span className={commands?.extractor || commands?.condenser ? "text-sky-300" : "text-slate-600"}>●</span> VAPOR LINE</div><div><span className={commands?.cooling ? "text-blue-300" : "text-slate-600"}>●</span> COOLING LINE</div><div><span className={commands?.heater ? "text-amber-300" : "text-slate-600"}>●</span> POWER / HEAT</div></div></div>
    <div className="absolute bottom-3 left-3 right-3 grid grid-cols-2 gap-2 md:grid-cols-6"><div className="rounded-lg border border-cyan-500/20 bg-slate-950/85 p-2 font-mono backdrop-blur"><div className="text-[8px] text-slate-500">REACTOR</div><div className="text-sm text-cyan-300">{machine ? `${machine.sensors.temperatureC.toFixed(1)} °C` : "—"}</div></div><div className="rounded-lg border border-cyan-500/20 bg-slate-950/85 p-2 font-mono backdrop-blur"><div className="text-[8px] text-slate-500">PRESSURE</div><div className="text-sm text-cyan-300">{machine ? `${machine.sensors.pressureMbar.toFixed(1)} mbar` : "—"}</div></div>{[0,1,2,3].map(i=><div key={i} className="rounded-lg border border-slate-800 bg-slate-950/85 p-2 font-mono backdrop-blur"><div className="text-[8px] text-slate-500">COLD TRAP {i+1}</div><div className="text-sm text-sky-300">{typeof traps[i]?.temperatureC === "number" ? `${traps[i].temperatureC.toFixed(1)} °C` : "—"}</div><div className="text-[8px] text-slate-500">CONDENSED {typeof traps[i]?.condensedWaterKg === "number" ? `${traps[i].condensedWaterKg.toFixed(3)} kg` : "—"}</div></div>)}</div>
  </div>;
}
