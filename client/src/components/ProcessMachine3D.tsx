...
export function getProcessMachineVisualState(frame?: CausalFrame): ProcessMachineVisualState {
  if (!frame) {
    return {
      ...
      materialRemainingKg: undefined,
      materialInitialKg: undefined,
      ultrasonicActivityIndex: undefined,
      ultrasonicEffectivePowerW: undefined,
    };
  }
...
export function ProcessMachine3D({ frame }: Props) {
  ...
  useEffect(() => {
    ...
    const floor = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color: 0x06101c, transparent: true, opacity: 0.92 }),
    );
    ...
    const top = new THREE.Mesh(
      new THREE.CylinderGeometry(1.79, 1.79, 0.3, 48),
      new THREE.MeshStandardMaterial({ color: 0x25374c, metalness: 0.9, roughness: 0.18 }),
    );
    top.position.y = 2.46;
    reactor.add(top);
    ...
    const coolingCurve = new THREE.CatmullRomCurve3([
      new THREE.Vector3(4.45, 1.62, -1.5),
      new THREE.Vector3(2.7, 1.62, -2.35),
      new THREE.Vector3(0, 1.7, -2.35),
      new THREE.Vector3(-2.05, 1.26, -0.82),
    ]);
    const powerCurve = new THREE.CatmullRomCurve3([
      new THREE.Vector3(5.75, -1.28, 2.58),
      new THREE.Vector3(4.5, -1.0, 2.58),
      new THREE.Vector3(2.0, 0, 1.9),
      new THREE.Vector3(-1.0, 0.2, 0.82),
    ...
    const animate = () => {
      ...
      flowParticles.children.forEach((particle, index) => {
        ...
        particle.scale.setScalar(active ? 0.7 + (thermal ?? 0) * 0.5 : 0);
    ...
    <div className="relative h-[590px] overflow-hidden rounded-2xl border border-cyan-500/20 bg-slate-950/90 shadow-2xl shadow-cyan-950/20">
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 flex items-start justify-between bg-gradient-to-b from-slate-950/95 via-slate-950/65 to-transparent p-4">
        ...
        <div className={`rounded-lg border px-3 py-2 font-mono text-[9px] ${fault ? "border-red-500/30 bg-red-500/10 text-red-300" : hasEngineFrame ? "border-emerald-500/20 bg-emerald-500/5 text-emerald-300" : "border-slate-700 bg-slate-950/80 text-slate-500"}`}>
          {fault ? "SAFETY TRIP" : hasEngineFrame ? visual.stage : "UNKNOWN / NO FRAME"}
        </div>
      </div>
      ...
      <div className="pointer-events-none absolute left-4 top-20 space-y-1.5 font-mono text-[9px]">
        ...
        <div className="rounded bg-slate-950/70 px-2 py-1 text-slate-400">MATERIAL / ULTRASONIC</div>
      ...
      <div className="pointer-events-none absolute right-4 top-20 rounded-xl border border-white/10 bg-slate-950/80 p-3 font-mono text-[9px] shadow-xl backdrop-blur">
        ...
        <div className="space-y-1.5 text-slate-400">
          <div><span className={commands?.vacuumPump ? "text-cyan-300" : "text-slate-600"}>●</span> VACUUM PATH</div>
          <div><span className={commands?.extractor || commands?.condenser ? "text-sky-300" : "text-slate-600"}>●</span> VAPOR PATH</div>
      ...
      <div className="absolute bottom-3 left-3 right-3 grid grid-cols-2 gap-2 md:grid-cols-6">
        <div className="rounded-lg border border-cyan-500/20 bg-slate-950/85 p-2 font-mono backdrop-blur">
          ...
          <div className="text-sm text-cyan-300">{displayNumber(visual.temperatureC, 1, "°C")}</div>
        </div>
        <div className="rounded-lg border border-cyan-500/20 bg-slate-950/85 p-2 font-mono backdrop-blur">
          <div className="text-[8px] text-slate-500">PRESSURE</div>
          <div className="text-sm text-cyan-300">{displayNumber(visual.pressureMbar, 1, "mbar")}</div>
        </div>
        {[0, 1, 2, 3].map(index => (
          <div key={index} className="rounded-lg border border-slate-800 bg-slate-950/85 p-2 font-mono backdrop-blur">
            ...
            <div className="text-sm text-sky-300">{displayNumber(traps?.[index], 1, "°C")}</div>
            <div className="text-[8px] text-slate-500">CONDENSED {displayNumber(condensed?.[index], 3, "kg")}</div>
          </div>
        ))}
      </div>
