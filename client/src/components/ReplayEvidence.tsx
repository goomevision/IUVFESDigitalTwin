...
type EvidencePackage = {
  ...
  provenance: {
    origin: "DIGITAL_TWIN_SIMULATION";
    laboratoryObservation: false;
    validationStatus: "NOT_LAB_VALIDATED";
...
function canonicalPayload(pkg: Omit<EvidencePackage, "integrity">) {
  return JSON.stringify(stable(pkg));
}
...
export function ReplayEvidence({
  ...
  experimentId: string;
  sessionId: string | null;
  frames: ReadonlyArray<ReplayEvidenceFrame>;
  ...
  const range = useMemo(() => {
    ...
    const lo = Math.min(start, end, frames.length - 1);
    const hi = Math.max(start, end, 0);
    return frames.slice(lo, Math.min(hi, frames.length - 1) + 1);
  }, [frames, start, end]);
  ...
  const capture = async (selectedOnly: boolean) => {
    ...
    try {
      ...
      const source = {
        experimentId,
        sessionId,
        frameStart: first.step,
        frameEnd: last.step,
        frameCount: selected.length,
        timestampStartSeconds: first.timestampSeconds,
        timestampEndSeconds: last.timestampSeconds,
      };
      const base = {
        ...
        source,
        provenance: {
          origin: "DIGITAL_TWIN_SIMULATION" as const,
          laboratoryObservation: false as const,
          validationStatus: "NOT_LAB_VALIDATED" as const,
  ...
  const download = () => {
    ...
    anchor.href = url;
    anchor.download = `iuvfes-replay-evidence-${evidence?.source.frameStart}-${evidence?.source.frameEnd}.json`;
    anchor.click();
  ...
  return <section className="rounded-2xl border border-emerald-500/20 bg-slate-950/75 p-4">
    <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
      <div><div className="text-[9px] tracking-[0.25em] text-slate-500">REPLAY → EVIDENCE</div><h3 className="font-semibold tracking-wider text-emerald-300">SCIENTIFIC EVIDENCE CAPTURE</h3></div>
      <div className="font-mono text-[9px] text-slate-600">SCHEMA IUVFES-REPLAY-EVIDENCE-1</div>
    </div>
    {!frames.length ? <div className="rounded-xl border border-dashed border-slate-800 p-6 text-center font-mono text-xs text-slate-600">WAITING FOR REPLAY FRAMES</div> : <>
      ...
      <div className="mt-3 rounded-xl border border-slate-800 bg-slate-950/60 p-3">
        <div className="mb-2 flex flex-wrap items-center justify-between gap-2 font-mono text-[9px]"><span className="text-slate-500">REPLAY CURSOR</span><span className="text-cyan-300">FRAME #{frames[safeSelected].step} · T+{frames[safeSelected].timestampSeconds.toFixed(1)} s</span></div>
        <input className="w-full accent-cyan-400" type="range" min={0} max={Math.max(0, frames.length - 1)} value={safeSelected} onChange={e => { const next = Number(e.target.value); onSelectIndex?.(next); setStart(next); setEnd(next); }}/>
      </div>
    </>}
    {evidence && <div className="mt-4 rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-3">
      <div className="flex flex-wrap items-center justify-between gap-2"><div><div className="text-[8px] tracking-[0.2em] text-slate-600">EVIDENCE PACKAGE READY</div><div className="font-mono text-[10px] text-emerald-300">FRAMES {evidence.source.frameStart}–{evidence.source.frameEnd} · {evidence.source.frameCount} FRAME(S)</div></div><div className="flex gap-2"><Button size="sm" variant="outline" onClick={download}><Download className="mr-1 h-3.5 w-3.5"/>JSON</Button><Button size="sm" variant="outline" onClick={() => void copy()}>{copied ? <Check className="mr-1 h-3.5 w-3.5"/> : <Clipboard className="mr-1 h-3.5 w-3.5"/>}{copied ? "COPIED" : "COPY"}</Button></div></div>
      <div className="mt-3 break-all rounded bg-slate-950/70 p-2 font-mono text-[8px] text-slate-500">SHA-256 · {evidence.integrity.canonicalPayloadSha256}</div>
      <div className="mt-2 text-[8px] leading-relaxed text-slate-600">SIMULATION-DERIVED · NOT LAB VALIDATED · source experiment {evidence.source.experimentId} · session {evidence.source.sessionId ?? "none"}</div>
    </div>}
  </section>;
}
