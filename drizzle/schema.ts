...
// Scientific research persistence layer.
export const researchExperiments = mysqlTable("researchExperiments", {
  ...
  title: varchar("title", { length: 255 }).notNull(),
  status: mysqlEnum("status", ["draft", "ready", "running", "paused", "completed", "failed", "reviewed"]).default("draft").notNull(),
  researcherId: varchar("researcherId", { length: 128 }).notNull(),
  objective: text("objective").notNull(),
  hypothesis: text("hypothesis"),
  materialId: int("materialId").notNull(),
  sampleId: varchar("sampleId", { length: 128 }).notNull(),
  batchId: varchar("batchId", { length: 128 }),
  massKg: decimal("massKg", { precision: 12, scale: 4 }).notNull(),
  environment: json("environment"),
  procedure: json("procedure").notNull(),
  inputParameters: json("inputParameters").notNull(),
  provenanceId: varchar("provenanceId", { length: 128 }),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
...
export const experimentInstruments = mysqlTable("experimentInstruments", {
  id: int("id").autoincrement().primaryKey(),
...
export const sensorObservations = mysqlTable("sensorObservations", {
  id: int("id").autoincrement().primaryKey(),
  experimentId: varchar("experimentId", { length: 64 }).notNull(),
  ...
  qualityFlag: mysqlEnum("qualityFlag", ["RAW", "VALIDATED", "REJECTED", "CORRECTED"]).default("RAW").notNull(),
  rawPayloadRef: varchar("rawPayloadRef", { length: 512 }),
  rawPayloadSha256: varchar("rawPayloadSha256", { length: 64 }),
}, (table) => ({ experimentTimeIdx: index("sensorObservations_experiment_time_idx").on(table.experimentId, table.observedAt) }));
...
export const operatorObservations = mysqlTable("operatorObservations", {
  id: int("id").autoincrement().primaryKey(),
  experimentId: varchar("experimentId", { length: 64 }).notNull(),
  observedAt: timestamp("observedAt").notNull(),
...
}, (table) => ({ experimentTimeIdx: index("operatorObservations_experiment_time_idx").on(table.experimentId, table.observedAt) }));
...
export const datasetManifests = mysqlTable("datasetManifests", {
  id: varchar("id", { length: 128 }).primaryKey(),
  experimentId: varchar("experimentId", { length: 64 }),
  version: varchar("version", { length: 32 }).notNull(),
  origin: mysqlEnum("origin", ["EXPERIMENTAL", "SIMULATION", "DERIVED", "AI_ANALYSIS"]).notNull(),
  qualityStatus: mysqlEnum("qualityStatus", ["RAW", "VALIDATED", "REVIEWED", "CALIBRATED", "REPLICATED", "PUBLISHED", "RETRACTED", "SUPERSEDED"]).notNull(),
  sha256: varchar("sha256", { length: 64 }).notNull(),
  storageRef: varchar("storageRef", { length: 512 }).notNull(),
  metadata: json("metadata").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
}, (table) => ({ experimentIdx: index("datasetManifests_experiment_idx").on(table.experimentId) }));
...
export const provenanceRecords = mysqlTable("provenanceRecords", {
  id: varchar("id", { length: 128 }).primaryKey(),
  entityId: varchar("entityId", { length: 128 }).notNull(),
  activityId: varchar("activityId", { length: 128 }).notNull(),
  agentId: varchar("agentId", { length: 128 }).notNull(),
  inputRefs: json("inputRefs").notNull(),
  outputRefs: json("outputRefs").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
});
...
export const experimentsRelations = relations(experiments, ({ one, many }) => ({
  ...
  simulationResults: many(simulationResults), controlLogs: many(controlLogs), reports: many(reports),
}));
...
export const researchExperimentsRelations = relations(researchExperiments, ({ one, many }) => ({
  material: one(materials, { fields: [researchExperiments.materialId], references: [materials.id] }),
  instruments: many(experimentInstruments), observations: many(sensorObservations), operatorObservations: many(operatorObservations), datasets: many(datasetManifests),
}));
